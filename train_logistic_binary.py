...
def train(dataloader, val_loader, model, optimizer, device):
    ...
    for idx, batch in tqdm(enumerate(dataloader), total=num_batches):
        ...
        labels_ohe = F.one_hot(labels, num_classes=2)
        ...
        if idx % log_interval == 0:
            ...
            correct = 0
            total = 0
            class_predictions_counts = {class_label: 0 for class_label in range(2)}
            class_label_counts = {class_label: 0 for class_label in range(2)}
            with torch.no_grad():
...
if __name__ == "__main__":
    ...
    torch.save(model.state_dict(), "models/fully_trained_logistic_binary_model")
